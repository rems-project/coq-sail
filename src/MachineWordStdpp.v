...
Definition word n := bv (N.of_nat n).

Definition zeros n : word n := bv_0 (N.of_nat n).

Definition get_bit [n] (w : word n) i : bool := Z.testbit (bv_unsigned w) (Z.of_nat i).
...
specialize (bv_to_bits_lookup_Some w (n - i - 1) x).
...
Definition slice {m} n (w : word m) i : word n := bv_extract (N.of_nat i) _ w.
Definition update_slice {m n} (w : word m) i (v : word n) : word m :=
  bv_concat (N.of_nat m)
    (slice (m - n - i) w (i + n))
    (bv_concat (N.of_nat (i+n)) v (slice i w 0)).
...
Definition logical_shift_left  [n] (w : word n) i : word n := bv_shiftl w (N_to_word n (N.of_nat i)).
Definition logical_shift_right [n] (w : word n) i : word n := bv_shiftr w (N_to_word n (N.of_nat i)).
Definition arith_shift_right   [n] (w : word n) i : word n := bv_ashiftr w (N_to_word n (N.of_nat i)).
...
Definition reverse_endian [n] (w : word n) : word n :=
  let bytes := bv_to_little_endian (Z.div (Z.of_nat n) 8) 8 (bv_unsigned w) in
