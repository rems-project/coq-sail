...
(** A simple datastructure to represent values taken from a toml file, and machinery for updating
    types using them, and for converting to them. *)
...
(* And to turn a value into a toml value. *)

Class ToGeneric (T : Type) := {
  to_generic : T -> generic_value
}.

(* Instances for base types and helper functions for the Sail output. *)
...
#[export] Instance to_generic_unit : ToGeneric unit := {
  to_generic _ := GVString "()"
}.
...
#[export] Instance to_generic_Z : ToGeneric Z := {
  to_generic z := GVNumber z
}.
...
#[export] Instance update_bitvector {n} : GenericUpdate (mword n) := {
  generic_update gv _prev :=
    match gv with
    | GVNumber z =>
        if orb (z >=? pow2 n)%Z (z <? -pow2 (n - 1))%Z
        then Err "value too large for bitvector"
        else Ok (mword_of_int z)
    | GVString s =>
...
#[export] Instance to_generic_bitvector {n} : ToGeneric (mword n) := {
  to_generic bv := GVString (string_of_bits bv)
}.
...
(* There's no helper function for to_generic on enums because Sail generates all of it. *)
...
#[export] Instance to_generic_bool : ToGeneric bool := {
  to_generic b := GVString (if b then "true" else "false")
}.

(* In Rocq tuples are really nested pairs.  We avoid this in toml values; however, we don't have
   a general instance yet, so provide fixed instances for small tuples at present. *)
...
#[export] Instance to_generic_prod2 {T1 T2} `{ToGeneric T1} `{ToGeneric T2} : ToGeneric (T1 * T2) | 10 := {
  to_generic '(v1, v2) := GVArray [to_generic v1; to_generic v2]
}.
...
#[export] Instance to_generic_prod3 {T1 T2 T3} `{ToGeneric T1} `{ToGeneric T2} `{ToGeneric T3} : ToGeneric (T1 * T3 * T2) | 9 := {
  to_generic '(v1, v2, v3) := GVArray [to_generic v1; to_generic v2; to_generic v3]
}.
...
#[export] Instance to_generic_prod4 {T1 T2 T3 T4} `{ToGeneric T1} `{ToGeneric T2} `{ToGeneric T3} `{ToGeneric T4} : ToGeneric (T1 * T2 * T3 * T4) | 8 := {
  to_generic '(v1, v2, v3, v4) := GVArray [to_generic v1; to_generic v2; to_generic v3; to_generic v4]
}.
...
#[export] Instance to_generic_prod5 {T1 T2 T3 T4 T5} `{ToGeneric T1} `{ToGeneric T2} `{ToGeneric T3} `{ToGeneric T4} `{ToGeneric T5} : ToGeneric (T1 * T2 * T3 * T4 * T5) | 7 := {
  to_generic '(v1, v2, v3, v4, v5) := GVArray [to_generic v1; to_generic v2; to_generic v3; to_generic v4; to_generic v5]
}.
...
#[export] Instance to_generic_prod6 {T1 T2 T3 T4 T5 T6} `{ToGeneric T1} `{ToGeneric T2} `{ToGeneric T3} `{ToGeneric T4} `{ToGeneric T5} `{ToGeneric T6} : ToGeneric (T1 * T2 * T3 * T4 * T5 * T6) | 6 := {
  to_generic '(v1, v2, v3, v4, v5, v6) := GVArray [to_generic v1; to_generic v2; to_generic v3; to_generic v4; to_generic v5; to_generic v6]
}.
...
#[export] Instance to_generic_option {T} `{ToGeneric T} : ToGeneric (option T) := {
  to_generic v := match v with None => GVString "None" | Some v' => GVArray [GVString "Some"; to_generic v'] end
}.
...
#[export] Instance to_generic_list {T} `{ToGeneric T} : ToGeneric (list T) := {
  to_generic l := GVArray (List.map to_generic l)
}.
...
#[export] Instance to_generic_vec {T} {n} `{ToGeneric T} : ToGeneric (vec T n) := {
  to_generic l := GVArray (List.map to_generic (projT1 l))
}.
...
#[export] Instance to_generic_string : ToGeneric string := {
  to_generic s := GVString s
}.
