...
Require Import Real Base GenericValue.
...
#[export] Instance ToGeneric_Access_strength : ToGeneric Access_strength := {
  to_generic v := match v with AS_normal => GVString "AS_normal" | AS_rel_or_acq => GVString "AS_rel_or_acq" | AS_acq_rcpc => GVString "AS_acq_rcpc" end
}.
...
| _ => Err ("Unknown field " ++ name)%string
end.
#[export] Instance update_Explicit_access_kind : GenericUpdate Explicit_access_kind := {
  generic_update up prev := match up with
  | GVStruct l => fold_left (fun x '(fl, v) => result_bind (update_Explicit_access_kind_field fl v) x) l (Ok prev)
  | _ => Err "Bad generic value for structure"
  end
}.
#[export] Instance to_generic_Explicit_access_kind : ToGeneric Explicit_access_kind := {
  to_generic v := GVStruct [
    ("variety", to_generic v.(Explicit_access_kind_variety));
    ("strength", to_generic v.(Explicit_access_kind_strength))
  ]
}.
...
Instance GenericUpdate_Access_kind `{GenericUpdate arch_ak} `{Inhabited arch_ak} : GenericUpdate (Access_kind arch_ak) := {
  generic_update up prev := match up with
  | GVArray [GVString "AK_explicit"; ak_up] => result_bind (fun x => Ok (AK_explicit x)) (generic_update ak_up (match prev with AK_explicit ak_prev => ak_prev | _ => inhabitant end))
  | GVString "AK_ifetch" => Ok (AK_ifetch tt)
  | GVString "AK_ttw" => Ok (AK_ttw tt)
  | GVArray [GVString "AK_arch"; ak_up] => result_bind (fun x => Ok (AK_arch x)) (generic_update ak_up (match prev with AK_arch ak_prev => ak_prev | _ => inhabitant end))
  | _ => Err "Invalid value for Access_kind"
  end
}.
#[export] Instance to_generic_Access_kind `{ToGeneric arch_ak} : ToGeneric (Access_kind arch_ak) := {
  to_generic v := match v with
    | AK_explicit x => GVArray [GVString "AK_explicit"; to_generic x]
    | AK_ifetch _ => GVString "AK_ifetch"
    | AK_ttw _ => GVString "AK_ttw"
    | AK_arch ak => GVArray [GVString "AK_arch"; to_generic ak]
    end
}.
